...
export default class ImgElement extends SuperElement {
    ...
    getControls() {
        ...
        var width_fix_checked = !element.obj[0].style.width.includes('%');

        return [
            new ImageControl(getI18n('align'), {
                width: element.obj.width() / element.obj.parent().width() * 100,
                width_fix_checked: width_fix_checked,
                width_fix: element.obj.width(),
                ...
                setWidth: function(width) {
                    ...
                    if (currentEditor.emailMode) {
                        element.obj.css('max-width', width);
                        element.obj.prop('width', width.replace('px', ''));
                    }

                    currentEditor.select(element);
                    currentEditor.handleSelect();
                },
                setWidthFix: function(width) {
                    element.obj.css('width', width);
                ...
                setHeight: function(height) {
                    ...
                    if (currentEditor.emailMode) {
                        element.obj.css('max-height', width);
                        element.obj.prop('height', width.replace('px', ''));
                    }
                ...
                setUrl: function(url) {
                    element.obj.removeAttr("src").attr('src', url);
