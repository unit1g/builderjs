...
export default class BackgroundImageControl extends Control {
    ...
    afterRender() {
        ...
        thisControl.toggleControls();

        // default values
        if (thisControl.hasImage()) {
            $(thisControl.selector).find('.background-image').attr('src', thisControl.value.image.split(/"/)[1].replace(/"/g, ''));
        }

        var bg_color = thisControl.value.color == 'rgba(0, 0, 0, 0)' ? '--' : thisControl.rgb2hex(thisControl.value.color);

        $(thisControl.selector).find('.background-color').val(bg_color);
        if (bg_color == '--') {
            $(thisControl.selector).find('.background-color[type=color]').val('#ffffff');
        }
        ...
        // Upload funcs
        var url_image_selected_origin = $("#builder_iframe").get(0).contentWindow.location;
        ...
        function saveImageUpload() {
            ...
            $.ajax({
                ...
                if (jqXHR.status == 200) {
                    var src = data.url;
                    var asrc = url_image_selected_origin + src.replace(url_image_selected_origin, '');
                    
                    $(thisControl.selector).find('.background-image').attr('src', asrc);
                    ...
                    thisControl.updateImageSrc(asrc);                    
                } else {
